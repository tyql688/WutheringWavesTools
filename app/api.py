...
def download_all_detail(type: str, resource_id_list: list):
    for resource_id in resource_id_list:
        res = _session.get(
            f"https://api.hakush.in/ww/data/zh/{type}/{resource_id}.json"
        )
        res.raise_for_status()
        with open(
            RAW_RESOURCE_PATH / f"{resource_id}.json", "w", encoding="utf-8"
        ) as f:
...
def download():
    ...
    download_all_item_pic(all_item, is_force=False)
    download_all_material_pic(all_item, is_force=False)
